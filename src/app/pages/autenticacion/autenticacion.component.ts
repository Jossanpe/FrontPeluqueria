...
import {
  ...
  FormGroup,
  Validators,
} from '@angular/forms'; //formularios reactivos, crea formularios facilmente y validaciones
...
import { AutorizacionService } from '../../core/services/AutorizacionService/autorizacion.service';
...
@Component({
  ...
  imports: [
    ...
    FooterComponent,
    HeaderComponent,
  ], //imports que afectan y se tienen que activar en el HTML
...
export class AutenticacionComponent {
  ...
  get tel() {
    return this.formularioAutenticacion.get('tel');
  }
  get claveSeguridad() {
    return this.formularioAutenticacion.get('claveSeguridad');
  }
    ...
    this.autenticacionService
      ...
      .subscribe({
        next: (response: AutenticacionResponse) => {
          //GUARDAR TOKEN
         this.servicioAutorizacion.guardarToken(response.token);

          //OBTENER ROL, TIENE EL METODO OBTNERPAYLOAD QUE USA EL JWTDECODE
          const rol = this.servicioAutorizacion.obtenerRol();
